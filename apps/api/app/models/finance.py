...
class PaymentOrder(Base):
 __tablename__="payment_orders";__table_args__=(UniqueConstraint("tenant_id","idempotency_key"),UniqueConstraint("tenant_id","provider_order_id"))
 id:Mapped[uuid.UUID]=mapped_column(UUID(as_uuid=True),primary_key=True,default=uid);tenant_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("tenants.id"),index=True,nullable=False);invoice_id:Mapped[uuid.UUID]=mapped_column(ForeignKey("invoices.id"),index=True,nullable=False);provider:Mapped[str]=mapped_column(String(40),nullable=False);provider_order_id:Mapped[str]=mapped_column(String(160),nullable=False);idempotency_key:Mapped[str]=mapped_column(String(160),nullable=False);amount:Mapped[Decimal]=mapped_column(Numeric(12,2),nullable=False);status:Mapped[str]=mapped_column(String(30),default="CREATED",nullable=False);created_at:Mapped[datetime]=mapped_column(DateTime(timezone=True),default=now,nullable=False)
class Payment(Base):
