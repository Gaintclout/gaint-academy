...
def confirm(proposal_id:UUID,u:User=Depends(current_user),db:Session=Depends(get_db)):
 ...
 x.status="CONFIRMED";db.add(AuditEvent(tenant_id=u.tenant_id,user_id=u.id,action="ai.action.confirmed",resource_type="ai_action_proposal",resource_id=str(x.id)));db.commit();return {"data":{"id":str(x.id),"status":x.status,"executed":False}}
