...
export default function Student360(){
 ...
 const [s,setS]=useState<any>(null); const [guardians,setGuardians]=useState<any[]>([]); const [me,setMe]=useState<any>(null);
 ...
 const [account,setAccount]=useState({guardian_id:"",email:"",password:""}); const [studentAccount,setStudentAccount]=useState({email:"",password:""});
 ...
 async function createStudentAccount(e:FormEvent){e.preventDefault();setError("");try{await apiFetch("/students/"+id+"/student-account",{method:"POST",body:JSON.stringify(studentAccount)});setStudentAccount({email:"",password:""});setMsg("Student login created with STUDENT scope.");await load()}catch(e){setError(e instanceof Error?e.message:"Unable to create student account")}}

 if(!s)return <main className="loading">Loading Student 360…</main>;
 const canAdmin=me?.permissions?.includes("students.student.create")??false;
 const canProvision=me?.permissions?.includes("users.user.create")??false;
 const canEnroll=me?.permissions?.includes("academics.setup.admin")??false;
 ...
 {canEnroll&&!s.enrollment&&<section className="panel settings-note"><h2>Enroll student</h2><form onSubmit={submitEnrollment}>
 ...
 {canProvision&&<form onSubmit={createAccount}><h3>Create parent login</h3><label>Guardian<select value={account.guardian_id} onChange={e=>setAccount({...account,guardian_id:e.target.value})} required><option value="">Select guardian</option>{guardians.filter(x=>!x.user_id).map(x=><option key={x.guardian_id} value={x.guardian_id}>{x.name}</option>)}</select></label><label>Login email<input type="email" value={account.email} onChange={e=>setAccount({...account,email:e.target.value})} required/></label><label>Temporary password<input type="password" minLength={10} value={account.password} onChange={e=>setAccount({...account,password:e.target.value})} required/></label><button>Create parent login</button></form>}</section></section>
 {canProvision&&<section className="panel settings-note"><h2>Create student login</h2><p className="muted">Creates a STUDENT account scoped only to this Student 360 record.</p><form onSubmit={createStudentAccount}><label>Login email<input type="email" value={studentAccount.email} onChange={e=>setStudentAccount({...studentAccount,email:e.target.value})} required/></label><label>Temporary password<input type="password" minLength={10} value={studentAccount.password} onChange={e=>setStudentAccount({...studentAccount,password:e.target.value})} required/></label><button>Create student login</button></form></section>}
 </main>;
}
